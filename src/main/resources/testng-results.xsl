...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="testNgXslt.maxDuration"/>
    <xsl:param name="testNgXslt.startAtTime"/>
    <xsl:param name="testNgXslt.numberReducation"/>
    ...
    <xsl:template name="htmlHead">
        <head>
            ...
            <meta http-equiv="Content-Security-Policy" content="default-src *; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline' 'unsafe-eval' http://10.30.5.91:8080" />
	...
	<xsl:function name="testng:trimException">
        <xsl:param name="arg"/>
        <xsl:sequence select="substring-after($arg,':')"/>
    </xsl:function>
    ...
    <xsl:function name="testng:suiteMethodsDuration">
        <xsl:param name="testCasesElements"/>
        <xsl:value-of
                select="sum($testCasesElements/class/test-method/@duration-ms)"/>
    </xsl:function>

    <xsl:function name="testng:suiteMethodsStartAt">
        <xsl:param name="testCasesElements"/>
        <xsl:value-of
                select="min($testCasesElements/class/test-method/@started-at)"/>
    </xsl:function>
    ...
    <xsl:template match="/testng-results">
		...
		<xsl:result-document href="{testng:absolutePath('summary.html')}" format="xhtml">
			...
			<xsl:call-template name="htmlHead"/>
				...
				<xsl:variable name="failedCount" select="testng:suiteMethodsCount($testCaseElements, 'FAIL')"/>
                <xsl:variable name="passedCount" select="testng:suiteMethodsCount($testCaseElements, 'PASS') - $testNgXslt.numberReducation"/>
                <xsl:variable name="skippedCount" select="testng:suiteMethodsCount($testCaseElements, 'SKIP')"/>
                <xsl:variable name="totalCount" select="testng:suiteMethodsCount($testCaseElements, '*') - $testNgXslt.numberReducation"/>
                <xsl:variable name="totalDuration" select="testng:formatDuration(testng:suiteMethodsDuration($testCaseElements))"/>
				<body>
					<h2>Test Result Summary</h2>
					<table id="summarytable" width="40%">
                        <tr>
                            <td style="text-align:left">Execution time:</td><td style="text-align:left"><xsl:value-of select="$testNgXslt.startAtTime"/></td>
                        </tr>
                        <tr>
                            <td style="text-align:left">Total execution time:</td><td style="text-align:left"><xsl:value-of select="$totalDuration"/></td>
                        </tr>
                        <tr>
                            <td style="text-align:left">Executed:</td><td style="text-align:left"><xsl:value-of select="$totalCount"/></td>
                        </tr>
                        <tr>
                            <td style="text-align:left">Passed:</td><td style="text-align:left"><xsl:value-of select="$passedCount"/></td>
                        </tr>
                        <tr>
                            <td style="text-align:left">Failed:</td><td style="text-align:left"><xsl:value-of select="$failedCount"/></td>
                        </tr>
                    </table>
				</body>
			</html>
                    ...
                    <xsl:for-each select="$suiteElements">
                        ...
                        <table width="100%" cellpadding="5" cellspacing="1">
                            ...
                            <xsl:for-each select="$testCaseElements">
                            	<xsl:if test = "testng:testCaseMethodsCount(., '*') > 0" >  <!-- add by Kevin Li, display only executed test cases (with data) -->
        ...
        <div id="testMethodsByStatus">
            ...
            <table class="testMethodsTable" cellpadding="0" cellspacing="0">
                <tr class="methodsTableHeader">
                    <td width="20%">Case Name</td>
                    <td width="10%" nowrap="true">Started</td>
                    <td width="10%" nowrap="true">Duration</td>                    
					<td width="20%">Detail Steps</td>
					<td width="40%">Detail Log</td>
                </tr>
        ...
        <xsl:for-each select="$methodList">
            ...
            <tr id="{concat($methodId, '_row')}" class="{testng:testMethodStatus(.)}">
                ...
                <td nowrap="true" align="right">
                	<xsl:choose>
                		<xsl:when test="@duration-ms > $testNgXslt.maxDuration">
                			<span style="color:#B35900"><b><xsl:value-of select="testng:formatDuration(@duration-ms)"/></b></span>
                		</xsl:when>
                		<xsl:otherwise>
		                    <xsl:value-of select="testng:formatDuration(@duration-ms)"/>
                		</xsl:otherwise>
                ...
                <td nowrap="true">
                    <xsl:if test="./reporter-output">
                        <a onclick="toggleDetailsVisibility('{$outputDetailsId}')">
                            Output
                        </a>
                    </xsl:if>
                    &#160;
            ...
            <tr>
                <xsl:if test="reporter-output">
                    <td colspan="5" class="detailsBox">
                        <div id="{$outputDetailsId}" class="testMethodDetails">
							<xsl:for-each select="reporter-output/line">
								<div>
									<code>
										<xsl:value-of select="." disable-output-escaping="yes"/>
                            ...
                            <xsl:choose>
                                <xsl:when test="exception/full-stacktrace">
                                    <pre style="padding: 5px; margin: 0;">
                                        <!--xsl:value-of select="testng:trimException(testng:trim(exception/full-stacktrace))"/-->
                                ...
                                <xsl:when test="exception/short-stacktrace and not (exception/full-stacktrace)">
                                    <pre style="padding: 5px; margin: 0;">
                                        <!--xsl:value-of select="testng:trimException(testng:trim(exception/short-stacktrace))"/-->
    ...
    <xsl:template name="suiteTestCasesLinks">
        ...
        <xsl:for-each select="$testCases">
            ...
            <xsl:if test = "testng:testCaseMethodsCount(., '*') > 0" > <!-- Added by Kevin Li, only display executed test case (with test data) -->
